...
export const Navbar = ({ activeView, setView, onPlusClick, userRole, hasUnread }: { activeView: View; setView: (v: View) => void; onPlusClick: () => void, userRole?: string, hasUnread?: boolean }) => {
    ...
    return (
        <nav className="fixed bottom-0 left-0 right-0 glass border-t border-white/5 safe-bottom z-50 rounded-t-[20px]">
            <div className="flex justify-around items-center h-20 px-4">
                {userRole === 'artisan' ? (
                    <>
                        <NavItem icon={<Home />} label={t('nav.stats')} active={activeView === 'home'} onClick={() => setView('home')} />
                        <NavItem icon={<Zap />} label={t('nav.marketplace')} active={activeView === 'marketplace'} onClick={() => setView('marketplace')} />
                        <NavItem icon={<MessageSquare />} label={t('nav.messages')} active={activeView === 'chats'} onClick={() => { setView('chats'); }} badge={hasUnread} />
                        <NavItem icon={<User />} label={t('nav.profile')} active={activeView === 'profile'} onClick={() => setView('profile')} />
                    </>
                ) : (
                    <>
                        <NavItem icon={<Home />} label={t('nav.home')} active={activeView === 'home'} onClick={() => setView('home')} />
                        <NavItem icon={<Search />} label={t('nav.search')} active={activeView === 'search'} onClick={() => setView('search')} />
                        ...
                        <NavItem icon={<Calendar />} label={t('nav.missions')} active={activeView === 'bookings'} onClick={() => setView('bookings')} />
                        <NavItem icon={<User />} label={t('nav.me')} active={activeView === 'profile'} onClick={() => setView('profile')} />
                    </>
                )}
            </div>
        </nav>
...
const NavItem = ({ icon, label, active, onClick, badge }: { icon: React.ReactNode, label: string, active: boolean, onClick: () => void, badge?: boolean }) => (
    <button onClick={onClick} className={`flex flex-col items-center gap-1 transition-all ${active ? 'text-[#34d399]' : 'text-slate-500 hover:text-slate-300'}`}>
        <div className="relative">
            ...
            {badge && !active && <div className="absolute -top-1 -right-1 size-2 bg-purple-500 rounded-full animate-pulse border border-[#0a0a0c]"></div>}
        </div>
