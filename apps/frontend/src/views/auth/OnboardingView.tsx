...
export const OnboardingView: React.FC<Props> = ({ userProfile, onComplete }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!name || phone.length !== 13) {
            setError(phone.length !== 13 ? "Le numéro de téléphone doit contenir 9 chiffres après +212." : "Veuillez remplir tous les champs.");
            return;
        }
    ...
    return (
        <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-[#0a0a0c] relative overflow-hidden">
            ...
            <div className="flex flex-col items-center text-center mb-8 z-10">
                ...
                <h1 className="text-3xl font-black text-white tracking-tighter mb-1 uppercase">Bienvenue</h1>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="space-y-1">
                        ...
                        <div className="relative">
                            ...
                            <input type="tel" value={phone} onChange={handlePhoneChange} placeholder="+212 ..." className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 pl-12 pr-4 text-white text-xs focus:outline-none focus:border-purple-500/50 transition-all placeholder:text-slate-700" />
                        </div>
                    </div>
                    ...
                    <button type="submit" disabled={isLoading} className="w-full bg-gradient-to-r from-[#a855f7] to-[#ec4899] py-5 rounded-2xl text-white font-black text-sm uppercase tracking-[0.2em] shadow-xl active:scale-[0.98] transition-all flex items-center justify-center gap-3 mt-4 group">
                        {isLoading ? <Loader2 className="size-4 animate-spin" /> : <>Continuer <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" /></>}
                    </button>
                </form>
            </div>
        </div>
