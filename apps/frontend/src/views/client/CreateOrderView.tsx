  ...
  return (
    <div className="min-h-screen bg-[#0a0a0c] flex flex-col animate-in slide-in-from-bottom duration-500 pb-10">
      <header className="px-6 pt-12 pb-6 flex items-center gap-4 sticky top-0 bg-[#0a0a0c]/90 backdrop-blur-xl z-50 border-b border-white/5">
        ...
        <div>
          <h1 className="text-xl font-black text-white tracking-tight uppercase">{t('create_order.title', { category: category.name })}</h1>
      ...
      <div className="px-6 py-8 space-y-8 flex-1">
        ...
        {preSelectedArtisan && (
          <div className="bg-indigo-600/10 border border-indigo-500/20 p-4 rounded-3xl flex items-center gap-4 shadow-lg">
            ...
            <div>
              ...
              <p className="text-[10px] text-indigo-400 font-medium">{hideArtisanName ? t('create_order.expert_vork') : t('create_order.for_artisan', { name: preSelectedArtisan.name })}</p>
            </div>
          </div>
        )}

        <div className="space-y-4">
          <label className="text-xl font-bold text-white tracking-tight">{t('create_order.description')}</label>
          <textarea
            value={description}
        ...
        <div className="space-y-4">
          <label className="text-xl font-bold text-white tracking-tight">{t('create_order.photos')} <span className="text-slate-500 font-medium text-sm">({t('create_order.max_photos')})</span></label>
          <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2">
            ...
            {images.length < 4 && (
              ...
              >
                ...
                <span className="text-[10px] font-bold text-slate-500 group-hover:text-purple-400 uppercase tracking-widest">{t('common.add')}</span>
              </button>
            )}
            <input
              type="file"
        ...
        <div className="space-y-4">
          <label className="text-xl font-bold text-white tracking-tight">{t('create_order.city')}</label>
          <div className="relative group">
            ...
            <input
              type="text"
              readOnly
              value={isUsingRealGPS ? t('create_order.gps_active') : t('create_order.current_location')}
        ...
        <div className="pt-4 mt-auto">
          ...
          >
            <div className="flex items-center justify-center gap-3">
              ...
              <span className="uppercase tracking-[0.1em]">{t('create_order.validate_with_ai')}</span>
            </div>
