...
export const VerifyEmailView: React.FC<Props> = ({ email, onBack, onLogout }) => {
    ...
    return (
        <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-[#0a0a0c] text-center">
            ...
            <h1 className="text-3xl font-black text-white tracking-tight mb-4 uppercase">{t('auth.verify_email_title')}</h1>
            <p className="text-slate-400 text-sm max-w-xs mb-8">
                {t('auth.verify_email_desc')} <span className="text-indigo-400 font-bold">{email}</span>.
            ...
            <div className="w-full max-w-xs space-y-4">
                ...
                >
                    {t('auth.i_verified')} <CheckCircle2 size={16} />
                </button>
