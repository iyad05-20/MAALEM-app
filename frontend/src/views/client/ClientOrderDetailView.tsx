...
const CompletionModal = ({ isOpen, onClose, onConfirm, artisanName, artisanImage, orderId, showToast }: {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setStep('rate');
            showToast(t('client_order_detail.messages.error_submit'), "error");
        }
    };
            ...
            <div className="relative w-full max-w-md bg-[#0a0a0c] border-t border-white/10 rounded-t-[3rem] p-8 pb-safe-bottom shadow-2xl animate-in slide-in-from-bottom duration-500 max-h-[90vh] overflow-y-auto no-scrollbar text-center">
                {step === 'rate' && (
                    <div className="space-y-6">
                        ...
                        <div>
                            <h2 className="text-xl font-black text-white uppercase tracking-tight">{t('client_order_detail.completion_modal.title')}</h2>
                            <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mt-1">{t('client_order_detail.completion_modal.subtitle', { name: artisanName })}</p>
                        </div>
                        ...
                        <div className="w-full space-y-3 text-left">
                            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest pl-1">{t('client_order_detail.completion_modal.result_photos')} <span className="text-indigo-400">{t('client_order_detail.completion_modal.required')}</span></label>
                            <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2">
                                {images.length < 4 && (
                                    <button onClick={() => fileInputRef.current?.click()} className="size-20 shrink-0 rounded-2xl border-2 border-dashed border-white/10 bg-white/5 flex flex-col items-center justify-center gap-1 group hover:border-indigo-500/50 transition-all">
                                        ...
                                        <span className="text-[8px] font-black text-slate-500 group-hover:text-indigo-400 uppercase tracking-widest text-center">{t('client_order_detail.completion_modal.add_photo')}</span>
                                    </button>
                                )}
                        ...
                        <textarea
                            ...
                            placeholder={t('client_order_detail.completion_modal.comment_placeholder')}
                        ...
                        <div className="flex w-full gap-3 pt-2">
                            <button onClick={onClose} className="flex-1 py-4 bg-white/5 rounded-2xl text-slate-500 font-black text-[10px] uppercase tracking-widest hover:bg-white/10">{t('client_order_detail.completion_modal.cancel')}</button>
                            <button onClick={handleSubmit} disabled={rating === 0} className="flex-[2] py-4 bg-emerald-500 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-emerald-500/20 disabled:opacity-50 disabled:cursor-not-allowed active:scale-95 transition-all">{t('client_order_detail.completion_modal.close_mission')}</button>
                        </div>
                    </div>
                )}
                ...
                {(step === 'uploading' || step === 'processing') && (
                    <div className="flex flex-col items-center justify-center py-10 space-y-6">
                        ...
                        <h3 className="text-white font-black uppercase tracking-widest animate-pulse">{step === 'uploading' ? t('client_order_detail.completion_modal.uploading') : t('client_order_detail.completion_modal.archiving')}</h3>
                    </div>
                )}
                ...
                {step === 'success' && (
                    <div className="flex flex-col items-center justify-center py-10 space-y-6 animate-in zoom-in duration-300">
                        ...
                        <h3 className="text-2xl font-black text-white uppercase tracking-tight">{t('client_order_detail.completion_modal.success')}</h3>
                    </div>
                )}
            </div>
        </div>
...
export const ClientOrderDetailView: React.FC<Props> = ({ order, onBack, onOpenChat, onOpenArtisanProfile, showToast }) => {
    ...
    const handleAcceptQuote = async (quote: Quote) => {
        ...
        if (!clientId) {
            showToast(t('client_order_detail.messages.error_client_id'), "error");
            setIsAccepting(null);
            return;
        }
        try {
            ...
            await setDoc(doc(db, "chats", chatDocId), {
                ...
                lastMessage: t('client_order_detail.messages.quote_accepted', { price: displayPrice }), timestamp: new Date().toISOString(),
            ...
            await addDoc(collection(db, "chats", chatDocId, "messages"), {
                text: t('client_order_detail.messages.initial_chat', { price: displayPrice }),
                sender: 'user', timestamp: new Date().toISOString(), status: 'sent'
            });
            await addDoc(collection(db, "notifications"), {
                userId: quote.artisanId,
                title: t('client_order_detail.messages.mission_notif_title'),
                message: t('client_order_detail.messages.mission_notif_body', { price: displayPrice, category: t(`categories.${order.category.toLowerCase().replace(/\s+/g, '_')}`) || order.category }),
                type: 'order_accepted',
                read: false,
    ...
    const handleConfirmCompletion = async (rating: number, comment: string, images: string[]) => {
        try {
            ...
            if (order.artisanId) {
                ...
                await addDoc(collection(db, "chats", chatDocId, "messages"), {
                    text: t('client_order_detail.messages.mission_done_chat', { rating }),
                    sender: 'user', timestamp: new Date().toISOString(), status: 'sent'
                });
                // Notify the artisan that the mission is complete
                await addDoc(collection(db, "notifications"), {
                    userId: order.artisanId,
                    title: t('client_order_detail.messages.mission_done_notif_title'),
                    message: t('client_order_detail.messages.mission_done_notif_body', { category: t(`categories.${order.category.toLowerCase().replace(/\s+/g, '_')}`) || order.category, rating }),
                    type: 'order_accepted',
                    read: false,
                    createdAt: new Date().toISOString(),
                    relatedId: order.id
    ...
    const handleExpandSearch = async () => {
        try {
            ...
            setCanExpand(false);
            showToast(t('client_order_detail.messages.search_expanded'), "success");
        } catch (err) { console.error(err); showToast(t('client_order_detail.messages.error_expand'), "error"); }
    };
    ...
    return (
        <div className="min-h-screen bg-[#0a0a0c] flex flex-col animate-in slide-in-from-right duration-500 pb-32">
            <header className="px-6 pt-12 pb-6 flex items-center justify-between sticky top-0 bg-[#0a0a0c]/90 backdrop-blur-xl z-50 border-b border-white/5">
                ...
                <h1 className="text-[11px] font-black text-white tracking-[0.2em] uppercase">{t('client_order_detail.header_title', { id: order.id.slice(-5).toUpperCase() })}</h1>
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar space-y-8 p-6 bg-[#0a0a0c]">
                ...
                <div className="glass-card p-6 rounded-[2.5rem] bg-[#1a1a20]/60 border border-white/5">
                    <div className="flex items-center gap-5">
                        ...
                        <div className="flex-1">
                            ...
                            <div className="flex items-center gap-2 text-slate-400">
                                <span className="text-[10px] font-black uppercase tracking-widest text-indigo-400">{t(`categories.${order.category.toLowerCase().replace(/\s+/g, '_')}`) || order.category}</span>
                    ...
                    <div className="flex items-center justify-between mt-6 pt-6 border-t border-white/5">
                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{t('orders.status')}</span>
                        <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border ${isAssigned ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-orange-500/10 border-orange-500/20 text-orange-400'}`}>
                            ...
                            <span className="text-[9px] font-black uppercase tracking-widest">{isPendingClosure ? t('client_order_detail.status_validation') : (t(`orders.status_types.${order.status.toLowerCase().replace(/\s+/g, '_')}`) || order.status)}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">{t('client_order_detail.description_title')}</h3>
                ...
                {(order.status === 'En cours' || order.status === 'Accepté') && (
                    <div className="glass-card p-6 rounded-[2rem] bg-indigo-900/10 border border-indigo-500/20 flex items-center justify-between shadow-xl">
                        <div className="relative z-10">
                            <h4 className="text-white font-black text-sm uppercase tracking-tight">{t('client_order_detail.work_finished_title')}</h4>
                            <p className="text-[10px] text-indigo-400/70 font-bold uppercase tracking-widest mt-0.5">{t('client_order_detail.work_finished_subtitle')}</p>
                        </div>
                        <button onClick={() => setShowCompletionModal(true)} className="px-6 py-3 bg-indigo-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg active:scale-95 transition-all flex items-center gap-2 hover:bg-indigo-500">
                            <CheckCircle2 size={16} /> {t('client_order_detail.validate_button')}
                        </button>
                    </div>
                )}
                ...
                {!isAssigned && (
                    <div className="space-y-6">
                        <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">{t('client_order_detail.quotes_received')}</h3>
                        {quotes.length > 0 ? (
                            <div className="space-y-4">
                                {quotes.map((quote) => (
                                    <div key={quote.id} className="glass-card p-6 rounded-[2.5rem] bg-[#121214] border border-white/10 shadow-2xl">
                                        ...
                                        <div className="flex gap-3">
                                            <button onClick={() => handleRejectQuote(quote)} disabled={isRejecting === quote.id || isAccepting !== null} className="flex-1 py-4 rounded-2xl bg-white/5 text-red-400 border border-white/5 font-black text-[11px] uppercase tracking-widest flex items-center justify-center gap-2">
                                                {isRejecting === quote.id ? <Loader2 className="size-4 animate-spin" /> : <><XCircle size={16} /> {t('client_order_detail.reject_button')}</>}
                                            </button>
                                            <button onClick={() => handleAcceptQuote(quote)} disabled={isAccepting !== null || isRejecting === quote.id} className="flex-[2] py-4 rounded-2xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-black text-[11px] uppercase tracking-widest flex items-center justify-center gap-2">
                                                {isAccepting === quote.id ? <Loader2 className="size-4 animate-spin" /> : <><UserCheck size={16} /> {t('client_order_detail.accept_button')}</>}
                                            </button>
                                        </div>
                                    </div>
                                ))}
                            ...
                            <div className="glass-card p-10 rounded-[2.5rem] bg-[#121214]/60 border border-dashed border-white/10 text-center">
                                ...
                                <p className="text-slate-600 font-black text-[10px] uppercase tracking-widest">{t('client_order_detail.searching_experts')}</p>
                                {canExpand && (
                                    <button onClick={handleExpandSearch} className="flex items-center gap-3 px-8 py-4 bg-indigo-600 text-white rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest shadow-xl mx-auto mt-4">
                                        <Zap size={14} className="fill-current" /> {t('client_order_detail.expand_search')}
                                    </button>
                                )}
                            </div>
                        )}
                ...
                {isAssigned && (
                    <div className="space-y-6">
                        <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">{t('client_order_detail.assigned_expert')}</h3>
                        <div onClick={() => onOpenArtisanProfile(order.artisanId)} className="glass-card p-6 rounded-[2.5rem] bg-[#121214] border border-white/10 shadow-2xl group cursor-pointer active:scale-[0.99] transition-all">
                            ...
                            <button onClick={(e) => { e.stopPropagation(); onOpenChat({ id: order.artisanId, name: order.artisanName, image: order.artisanImage }); }} className="w-full mt-8 py-4 bg-indigo-600 rounded-2xl flex items-center justify-center gap-3 text-white font-black text-[11px] uppercase tracking-widest active:scale-95 transition-all shadow-xl shadow-indigo-600/20">
                                <MessageCircle size={16} /> {t('client_order_detail.chat_with_expert')}
                            </button>
                        </div>
                    </div>
                )}
                ...
                {/* Photos Initiales */}
                <div className="space-y-4">
                    <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">{t('client_order_detail.initial_photos')}</h3>
                ...
                {order.resultImages && order.resultImages.length > 0 && (
                    <div className="space-y-4 pt-4 border-t border-white/5">
                        <h3 className="text-[10px] font-black text-emerald-500 uppercase tracking-widest px-1">{t('client_order_detail.final_result')}</h3>
                        <div className="grid grid-cols-4 gap-2.5">
